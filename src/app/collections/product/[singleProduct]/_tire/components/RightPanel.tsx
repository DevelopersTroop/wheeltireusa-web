...
const RightPanel = ({ product, selectedSize, setSelectedSize, availableSizes, qty, setQty }: RightPanelProps) => {
    ...
    <div className="p-5 sm:p-7 flex flex-col gap-4">
      ...
      <div className="flex justify-between items-start gap-3">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">
            {product?.tireClass || product?.category || "All Weather"} tire
          </p>
        </div>
        <StarRating rating={averageRating || 4.9} count={reviewCount || 17} />
      </div>
      ...
      <div className="flex items-center gap-4 sm:gap-5 text-[13px]">
        <span className="flex items-center gap-1.5 text-emerald-600 font-semibold">
          <MdCheckCircle className="w-3.5 h-3.5" /> In stock
        </span>
      ...
      <div className="flex gap-2.5">
        ...
        <button
          ...
          className={`flex-1 rounded-lg py-3 text-[15px] font-bold text-white transition-all duration-200 tracking-wide ${
            added
